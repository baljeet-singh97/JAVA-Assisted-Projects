...
public class ProductController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("productPrice").length() == 0) {
			errors.put("nullPrice","Enter Price<br/>");
		}
		try {
		...
		if(errors.isEmpty()) {
			...
			product p1 = new product(Integer.parseInt(request.getParameter("productId")), 
					request.getParameter("productName"), 
					request.getParameter("productCategory"), request.getParameter("productPrice"));
